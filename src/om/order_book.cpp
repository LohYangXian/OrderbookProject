    ...
    void matchOrders() {
        while (!bids.empty() && !asks.empty()) {
            ...
            if (bestBidPrice >= bestAskPrice) {
                ...
                // Log the trade
                Trade trade({bestBidPrice, tradeQty, bidOrder->getOrderId()},
                            {bestAskPrice, tradeQty, askOrder->getOrderId()});
                ...
                std::cout << std::format("Trade executed: {} @ ${:.2f} between Bid Order ID {} and Ask Order ID {}\n",
                                         tradeQty, bestAskPrice / 100.0, bidOrder->getOrderId(), askOrder->getOrderId());
